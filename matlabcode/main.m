...
bsbasis = createT1(30,3,-4,4);

% [Theta,PI]=fixedmove(T,Y);

[BIC,Theta,PI,A,B,component_num]=SMGPFRL1(T,Y,bsbasis);
...
load('data_test.mat');
 ...
 YP=mix_GPFRP_qz(Theta,PI,T1,Y1,T2,bsbasis,B);
...
abs((Theta_true(1:3,:) - Theta_sort)./Theta_true(1:3,:))*100
%__________________save data ________________________________________%
delete(['result',num2str(step),'.mat'])
save(['result',num2str(step),'.mat'])
